...
import PriceEditorSheet from './price-editor-sheet';
...
export default function LaunchGuideSheet() {
  ...
  const handleSaveChanges = (newPrices: Record<string, number>, newFee: number) => {
    setPredefinedPrices(newPrices);
  ...
  return (
    <>
      <PriceEditorSheet
        ...
        initialPrices={predefinedPrices}
      ...
      <Sheet open={isGuideOpen} onOpenChange={setIsGuideOpen}>
        <SheetTrigger asChild>
          <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground">
            ...
            <span className="sr-only">Guia e Definições</span>
          </Button>
        </SheetTrigger>
        <SheetContent className="w-[400px] sm:w-[540px]">
          ...
          <div className="absolute top-4 right-[4.5rem]">
            <Button variant="outline" size="sm" onClick={() => { setIsGuideOpen(false); setIsPriceEditorOpen(true); }}>
              ...
              <div className="py-6">
                  <Accordion type="single" collapsible className="w-full">
                      ...
                      <AccordionItem value="item-2">
                          ...
                          <AccordionContent>
                              <ul className="list-disc pl-5 space-y-2 mt-2 text-sm text-muted-foreground">
                                  ...
                                  <li><strong>Quantidade:</strong> Coloque o número antes da sigla, separado por espaço. Ex: <code className="bg-muted px-1.5 py-0.5 rounded-sm font-mono">2 M</code> (duas refeições médias).</li>
                      ...
                      <AccordionItem value="item-3">
                          ...
                          <AccordionContent>
                              <ul className="list-disc pl-5 space-y-2 mt-2 text-sm text-muted-foreground">
                                  <li><code>F 2 M G João</code>: Fiado no salão para João, 2 refeições M e 1 G.</li>
                                  ...
                                  <li><code>2 M P coca-lata 2</code>: Venda no salão, 2 refeições M, 1 P e 2 Cocas-Lata (preço padrão).</li>
                              </ul>
                          </AccordionContent>
                      </AccordionItem>
                  </Accordion>
