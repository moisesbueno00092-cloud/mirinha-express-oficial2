...
import { Loader2, ArrowLeft, User, Info, ShieldX } from 'lucide-react';
...
function FavoritosReportPageContent() {
  ...
  const [customerItems, setCustomerItems] = useState<Record<string, Item[]>>({});
  ...
  const fetchCustomerItems = useCallback(async () => {
    if (!firestore || !user || savedFavorites.length === 0) {
        setIsLoading(false);
        return;
    };
    setIsLoading(true);
    ...
    try {
        ...
        const customerNames = savedFavorites.map(fav => fav.name);
        ...
        snapshots.forEach(snapshot => {
            snapshot.forEach((doc) => {
                allItemsMap.set(doc.id, { ...doc.data(), id: doc.id } as Item);
            });
        });

        const itemsByCustomer: Record<string, Item[]> = {};
        ...
        // Sort items within each customer
        for (const customer in itemsByCustomer) {
            itemsByCustomer[customer].sort((a, b) => a.timestamp.toDate().getTime() - b.timestamp.toDate().getTime());
        }

        setCustomerItems(itemsByCustomer);

    } catch (error) {
        console.error("Error fetching customer items:", error);
        toast({
            variant: "destructive",
            title: "Erro ao buscar dados dos clientes",
            description: "Não foi possível carregar os itens. Verifique a sua conexão e as regras de segurança."
        });
    } finally {
        setIsLoading(false);
    }
  }, [firestore, user, selectedMonth, toast, savedFavorites]);

  useEffect(() => {
    if(isAuthenticated) {
        fetchCustomerItems();
    }
  }, [isAuthenticated, fetchCustomerItems]);

  const aggregatedCustomerData = useMemo(() => {
    return savedFavorites
      .map(favorite => {
        const items = customerItems[favorite.name] || [];
        ...
        <header className="mb-6 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground">Consumo mensal dos clientes favoritos para o mês selecionado.</p>
            </div>
          </div>
                 ...
                 <Card>
                    <CardContent className="p-10 text-center text-muted-foreground">
                        ...
                        {savedFavorites.length > 0 ? (
                           <p>Nenhum consumo encontrado para os clientes favoritos no mês de {format(selectedMonth, "MMMM 'de' yyyy", { locale: ptBR })}.</p>
                        ) : (
                           <p>Não existem clientes na sua lista de favoritos para gerar um relatório.</p>
                        )}
                    </CardContent>
                </Card>
            ) : (
                aggregatedCustomerData.map(customer => (
                    <Card key={customer.name}>
                        <CardHeader className="flex-row items-center justify-between">
                            ...
                            <div className="text-right">
                                ...
                                <p className="text-2xl font-bold text-primary">{formatCurrency(customer.total)}</p>
                            </div>
                        </CardHeader>
                        <CardContent>
                            ...
                            <div className="space-y-4">
                                {customer.items.map(item => (
                                    <div key={item.id} className="flex justify-between items-start text-sm p-2 rounded-md hover:bg-muted/50">
                                        <div>
                                            <div className="mb-1">{renderItemName(item)}</div>
...
export default function FavoritosReportPage() {
    return (
        <FavoritosReportPageContent />
    )
}
